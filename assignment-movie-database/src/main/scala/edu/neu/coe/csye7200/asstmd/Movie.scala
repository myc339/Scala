...
object Movie extends App {
  ...
  trait IngestibleMovie extends Ingestible[Movie] {
    ...
    def fromString(w: String): Try[Movie] = Try(w.split(','))
       match {
        case Success(info) =>Try(Movie(info))
  ...
  def elements(list: Seq[String], indices: Int*): List[String] = {
    ...
    for( ele<-indices collect list) yield x+=ele
...
object Rating {
  ...
  def apply(s: String): Rating = s match {
    case rRating(first,_,null)=>apply(first,None)
    case rRating(first,_,third)=>apply(first,Some(third.toInt))
