...
class LazyListSpec extends FlatSpec with Matchers {
  ...
  it should "take 3 from an infinite stream of 1s that counts" in {
    ...
    // Because lazylist will only evaluated when the valued is need,so at begin of initiate  lazylist, it will first intialize the head  of lazylist,so we
    // will get count as  1, then if we transform lazylist into seq, then the 3 value will need, so it  will evaluated   and  incrementCountAndProvideValue
    // will  be call 3 times, thus count =4
    count shouldBe 4
  }
