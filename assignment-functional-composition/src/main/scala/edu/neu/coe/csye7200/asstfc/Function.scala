...
object Function {
  ...
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] =for(t1<-t1y;t2<-t2y) yield f(t1,t2)
    //t1y match{
//    case Success(t1y) => t2y match{
//      case Success(t2y)=>Try(f(t1y,t2y))
//
//    }
//  } // TO BE IMPLEMENTED 4
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] ={ (t1,t2)=>map2(t1,t2)(f) }
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] = { (t1,t2,t3)=>map3(t1,t2,t3)(f) }
  /**
