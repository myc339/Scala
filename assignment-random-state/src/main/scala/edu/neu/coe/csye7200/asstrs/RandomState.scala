...
trait RandomState[T] {
  ...
  def toStream: Stream[T]=
    {
      def getElement(nele:RandomState[T]):Stream[T]={
        Stream(nele.get).append(getElement(nele.next))
      }
      getElement(this)
    }

   // TO BE IMPLEMENTED 12 points
}
...
object RandomState {
  ...
  val longToDouble: Long=>Double = {x=>x.toDouble/Double.MaxValue }// TO BE IMPLEMENTED 4 points
